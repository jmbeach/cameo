...
class FilterBackgroundBlur(Filter):
    ...
    def pixelize(frame, faces):
        ...
        feathered = FilterBackgroundBlur.alpha_blend(frame, blurred, feather_mask)
            ...
            for i, a in enumerate(faces):
                ...
                for j, b in enumerate(faces):
                    if j >= i:
                        continue
                    (x2, y2, w2, h2) = b
                    overlap = FilterBackgroundBlur.areas_overlap(a, b)
                    if overlap:
                        # remove both faces
                        for x in [a, b]:
                            try:
    ...
    def draw(self, frame):
        faces = FilterBackgroundBlur.detect_faces(frame, FilterBackgroundBlur.face_cascade)
        height, width, _ = frame.shape
        faces = FilterBackgroundBlur.increase_face_area(faces, width, height)
        faces = FilterBackgroundBlur.merge_faces(faces, width, frame)
        frame = FilterBackgroundBlur.pixelize(frame, faces)
        if self.debug:
            FilterBackgroundBlur.mark_faces(frame, faces)
